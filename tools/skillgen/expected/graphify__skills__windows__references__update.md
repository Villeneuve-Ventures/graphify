...
For an explicitly code-only update, use the public CLI through the saved interpreter:
...
& (Get-Content graphify-out\.graphify_python) -E -P -B -m graphify update INPUT_PATH
```
...
& (Get-Content graphify-out\.graphify_python) -E -P -B -m graphify cluster-only .
```
