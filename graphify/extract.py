...
@strict_aware
def _import_js(node, source: bytes, file_nid: str, stem: str, edges: list, str_path: str, scope_stack: list[str] | None = None, *, strict: bool = False) -> None:
...
@strict_aware
def _import_c(node, source: bytes, file_nid: str, stem: str, edges: list, str_path: str, scope_stack: list[str] | None = None, *, strict: bool = False) -> None:
    for child in node.children:
...
@strict_aware
def _import_lua(node, source: bytes, file_nid: str, stem: str, edges: list, str_path: str, scope_stack: list[str] | None = None, *, strict: bool = False) -> None:
    ...
    if m:
        ...
        if raw_module:
            tgt_nid = _resolve_lua_import_target(raw_module, str_path, strict=strict)
            if tgt_nid:
                edges.append({
                    "source": file_nid,
                    "target": tgt_nid,
...
def _extract_python_rationale(path: Path, result: dict, *, strict: bool = False) -> None:
...
@strict_aware
def extract_python(path: Path, *, strict: bool = False) -> dict:
    """Extract classes, functions, and imports from a .py file via tree-sitter AST."""
    result = _extract_generic(path, _PYTHON_CONFIG, strict=strict)
    if "error" not in result:
        _extract_python_rationale(path, result, strict=strict)
    return result


@strict_aware
def extract_js(path: Path, *, strict: bool = False) -> dict:
    ...
    if suffix == ".tsx":
        ...
        config = _JS_CONFIG
    result = _extract_generic(path, config, strict=strict)
    if "error" not in result:
        _extract_js_rationale(path, result, strict=strict)
    return result
...
def _extract_js_rationale(path: Path, result: dict, *, strict: bool = False) -> None:
...
@strict_aware
def extract_svelte(path: Path, *, strict: bool = False) -> dict:
    ...
    try:
        ...
        for m in _re.finditer(r"""import\(\s*['"]([^'"]+)['"]\s*\)""", src):
            ...
            if raw.startswith("."):
                ...
                resolved = _resolve_js_module_path(resolved, strict=strict)
                ...
                resolved_alias = _resolve_tsconfig_alias(raw, aliases, strict=strict)
                if resolved_alias is not None:
                    resolved_alias = _resolve_js_module_path(resolved_alias, strict=strict)
...
@strict_aware
def extract_astro(path: Path, *, strict: bool = False) -> dict:
...
@strict_aware
def extract_vue(path: Path, *, strict: bool = False) -> dict:
    ...
    result = _extract_generic(path, config, source_override=masked.encode("utf-8"), strict=strict)
...
@strict_aware
def extract_java(path: Path, *, strict: bool = False) -> dict:
    """Extract classes, interfaces, methods, constructors, and imports from a .java file."""
    return _extract_generic(path, _JAVA_CONFIG, strict=strict)


def _is_spock_file(path: Path, ts_result: dict, *, strict: bool = False) -> bool:
...
@strict_aware
def extract_groovy(path: Path, *, strict: bool = False) -> dict:
    ...
    """
    result = _extract_generic(path, _GROOVY_CONFIG, strict=strict)
    if strict and "error" in result:
        return result
    if _is_spock_file(path, result, strict=strict):
        result = _extract_spock_fallback(path, result)
    return result


@strict_aware
def extract_c(path: Path, *, strict: bool = False) -> dict:
    """Extract functions and includes from a .c/.h file."""
    return _extract_generic(path, _C_CONFIG, strict=strict)


@strict_aware
def extract_cpp(path: Path, *, strict: bool = False) -> dict:
    """Extract functions, classes, and includes from a .cpp/.cc/.cxx/.hpp file."""
    return _extract_generic(path, _CPP_CONFIG, strict=strict)


@strict_aware
def extract_ruby(path: Path, *, strict: bool = False) -> dict:
    """Extract classes, methods, singleton methods, and calls from a .rb file."""
    return _extract_generic(path, _RUBY_CONFIG, strict=strict)


@strict_aware
def extract_csharp(path: Path, *, strict: bool = False) -> dict:
    """Extract C# type declarations, methods, namespaces, and usings from a .cs file."""
    return _extract_generic(path, _CSHARP_CONFIG, strict=strict)


@strict_aware
def extract_kotlin(path: Path, *, strict: bool = False) -> dict:
    """Extract classes, objects, functions, and imports from a .kt/.kts file."""
    return _extract_generic(path, _KOTLIN_CONFIG, strict=strict)


@strict_aware
def extract_scala(path: Path, *, strict: bool = False) -> dict:
    """Extract classes, objects, functions, and imports from a .scala file."""
    return _extract_generic(path, _SCALA_CONFIG, strict=strict)


@strict_aware
def extract_php(path: Path, *, strict: bool = False) -> dict:
    """Extract classes, functions, methods, namespace uses, and calls from a .php file."""
    return _extract_generic(path, _PHP_CONFIG, strict=strict)
...
@strict_aware
def extract_lua(path: Path, *, strict: bool = False) -> dict:
    """Extract functions, methods, require() imports, and calls from a .lua file."""
    return _extract_generic(path, _LUA_CONFIG, strict=strict)


@strict_aware
def extract_swift(path: Path, *, strict: bool = False) -> dict:
    """Extract classes, structs, protocols, functions, imports, and calls from a .swift file."""
    return _extract_generic(path, _SWIFT_CONFIG, strict=strict)
...
def extract_lazarus_package(path: Path, *, strict: bool = False) -> dict:
    ...
    for item in xml_root.findall(".//Files/"):
        ...
        if unit_elem is not None:
            ...
            if unit_name:
                unit_nid = _pascal_resolve_unit(path, unit_name, strict=strict)
...
@strict_aware
def extract_slnx(path: Path, *, strict: bool = False) -> dict:
...
@strict_aware
def extract_csproj(path: Path, *, strict: bool = False) -> dict:
...
def _xaml_codebehind_path(path: Path, *, strict: bool = False) -> Path | None:
    expected = path.with_suffix(path.suffix + ".cs")
    if checked_exists(expected, strict=strict):
        return expected
    try:
        for sibling in (checked_glob(path.parent, "*", strict=True) if strict else path.parent.iterdir()):
...
def _xaml_codebehind_symbols(
    ...
    *,
    ...
    codebehind = _xaml_codebehind_path(path, strict=strict)
    if not codebehind:
        return None, {}, []
    result = extract_csharp(codebehind, strict=strict)
    if result.get("error"):
        if strict:
            raise RuntimeError(f"XAML companion extraction failed: {result["error"]}")
        return None, {}, []
...
def _xaml_csharp_class_nodes(path: Path, *, strict: bool = False) -> dict[str, list[dict]]:
    ...
    patterns = _load_graphifyignore(root, strict=strict)
    ignore_cache: dict[Path, bool] = {}
    try:
        cs_files = sorted(checked_rglob(root, "*.cs", strict=strict))
    except OSError:
        if strict:
            raise
        return classes
    for cs_path in cs_files:
        ...
        if patterns and _is_ignored(cs_path, root, patterns, _cache=ignore_cache):
            continue
        result = extract_csharp(cs_path, strict=strict)
        if result.get("error"):
            if strict:
                raise RuntimeError(f"XAML class extraction failed: {result["error"]}")
            continue
        for node in result.get("nodes", []):
            label = str(node.get("label", ""))
...
def _xaml_communitytoolkit_members(vm_node: dict, *, strict: bool = False) -> tuple[dict[str, dict], list[dict]]:
...
@strict_aware
def extract_xaml(path: Path, *, strict: bool = False) -> dict:
    ...
    class_node, codebehind_methods, class_method_edges = _xaml_codebehind_symbols(path, class_name, strict=strict)
    if class_name:
        if class_node:
    ...
    if vm_names:
        csharp_classes = _xaml_csharp_class_nodes(path, strict=strict)
        vm_candidates = []
        ...
        if len(by_id) == 1:
            ...
            generated_members, generated_member_edges = _xaml_communitytoolkit_members(vm_node, strict=strict)
...
def _is_objc_header(path: Path, *, strict: bool = False) -> bool:
...
def _is_objc_source(path: Path, *, strict: bool = False) -> bool:
    ...
    is not sniffed.
    """
    return _is_objc_header(path, strict=strict)


def _is_cpp_header(path: Path, *, strict: bool = False) -> bool:
...
def _get_extractor(path: Path, *, strict: bool = False) -> Any | None:
    ...
    if suffix not in _DISPATCH and suffix.lower() in _DISPATCH:
        suffix = suffix.lower()
    if suffix == ".h":
        if _is_objc_header(path, strict=strict):
            return extract_objc
        ...
        if _is_cpp_header(path, strict=strict):
            return extract_cpp
    ...
    if suffix == ".m" and not _is_objc_source(path, strict=strict):
        return None
    ...
    if not suffix:
        ...
        interp = _shebang_interpreter(path, strict=strict)
...
def _extract_parallel(
    ...
    try:
        ...
        if strict:
            raise RuntimeError("AST worker pool failed")
...
def extract(
    ...
    *,
    ...
    if strict and any(not isinstance(result, dict) or "error" in result
                      ...
                      or not isinstance(result.get("edges"), list) for result in per_file):
        raise RuntimeError("AST extraction incomplete: missing or failed source result")
    ...
    for i, _p in enumerate(paths):
        ...
        if _res.get("nodes") or _res.get("error"):
            continue
        if _get_extractor(_p, strict=strict) is not None:
    ...
    for _p in paths:
        ...
        if _ext in _CODE_EXTS and _get_extractor(_p, strict=strict) is None:
    ...
    _augment_symbol_resolution_edges(paths, all_nodes, all_edges, root, strict=strict)
    ...
    _disambiguate_colliding_node_ids(all_nodes, all_edges, all_raw_calls, root, strict=strict)
    ...
    if _php_sel:
        try:
            _resolve_php_type_references(
                [r for r, _ in _php_sel], [p for _, p in _php_sel], all_nodes, all_edges, strict=strict
            )
        except Exception as exc:
            if strict:
                raise
    ...
    if py_paths:
        ...
        try:
            cross_file_edges = _resolve_cross_file_imports(py_results, py_paths, strict=strict)
    ...
    if java_paths:
        ...
        try:
            all_edges.extend(_resolve_cross_file_java_imports(java_results, java_paths, strict=strict))
        except Exception as exc:
            if strict:
                raise
            import logging
        ...
        try:
            _resolve_java_type_references(java_results, java_paths, all_nodes, all_edges, strict=strict)
        except Exception as exc:
            if strict:
                raise
            import logging
