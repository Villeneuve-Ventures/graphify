"""Canonical internal lifecycle records for the v8 structural workspace.
...
_JOURNAL_MAGIC = b"GWF2"
_JOURNAL_FRAME_VERSION = 2
...
def _validate_generation_receipt(data: Mapping[str, object]) -> None:
    ...
    completion = CompletionBinding.from_mapping(
        _mapping(data["completion_binding"], "$.completion_binding")
    )
    completion_value = completion.to_dict()
    if (completion_value["initial_detection_sha256"] != data["observation_manifest_sha256"]
            or completion_value["compatibility_sha256"] != data["compatibility_sha256"]):
        raise ContractError("$.completion_binding: differs from receipt authority")
    ...
    if root != "graphify-out":
        raise ContractError("$.sealed_query_payload.root: requires 'graphify-out'")
    ...
    paths = {entry["path"]: entry for entry in cast(Sequence[Mapping[str, object]], payload["entries"])}
    if set(paths) != {"graphify-out/graph.json", "graphify-out/input-manifest.json"}:
    ...
    if validations != [
        ...
        raise ContractError("$.validations: structural certification proofs are required")
...
class ContractDocument:
    ...
    def from_mapping(cls, value: Mapping[str, object]) -> "ContractDocument":
        ...
        expected_version = 1 if raw_contract == "graphify.workspace.config" else WORKSPACE_SCHEMA_VERSION
        if raw_version != expected_version:
            raise UnsupportedContractVersion(
                f"$.schema_version: expected {expected_version}, got {raw_version}"
            )
        ...
        if version != expected_version:
            raise UnsupportedContractVersion(
                f"$.schema_version: expected {expected_version}, got {version}"
            )
...
class StagedBuildState:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "pointer_revision": self.pointer_revision,
            "completion_binding": (
                None if self.completion_binding is None else self.completion_binding.to_dict()
            ),
            "abandonment_intent": (
                None
    ...
    def from_mapping(cls, value: Mapping[str, object]) -> "StagedBuildState":
        ...
        _exact_version(data["format_version"], "$.format_version", 2)
        ...
        completion_binding = data["completion_binding"]
        if completion_binding is not None:
            completion_binding = CompletionBinding.from_mapping(
                _mapping(completion_binding, "$.completion_binding")
            )
            binding_value = completion_binding.to_dict()
            if (binding_value["initial_detection_sha256"] != request.observation_manifest_sha256
                    or binding_value["compatibility_sha256"] != request.compatibility_sha256):
                raise ContractError("$.completion_binding: differs from frozen request")
        ...
        if lifecycle_state == "REQUESTED":
            ...
            if completion_binding is not None:
                raise ContractError("$: REQUESTED staged build must not record completion binding")
            ...
            if (abandoned_from in {"REQUESTED", "PUBLISHING"}) != (completion_binding is None):
                raise ContractError("$: abandonment completion binding does not match prior state")
...
def decode_journal_frame(frame: bytes) -> JournalEvent:
    """Decode exactly one complete v2 frame and reject truncation or tampering."""
    ...
    if magic != _JOURNAL_MAGIC:
        raise ContractError("journal frame magic does not match GWF2")
