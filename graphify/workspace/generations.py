...
class GenerationStore:
        ...
        *,
        ...
        with self.state.existing_generation_lock(
            ...
            if self.semantic_queue is None or state.payload_manifest_sha256 is None:
                raise GenerationConflict("promoted queue authority is unavailable")
            ...
            certification_request = CertificationRequest(
                ...
                validations=tuple(str(item) for item in receipt_value["validations"]),
            )
            try:
                view = SemanticQueueStore.verify_certification_binding_at(
                    self.state, repo_uuid,
                    generation_id=generation_id,
                ...
                raise GenerationConflict(f"promoted queue binding is invalid: {exc}") from exc
            if (
                view.repo_uuid != repo_uuid
                or view.source_commit != request.source_commit
                or view.source_epoch != request.source_epoch
                or view.policy_sha256 != request.policy_sha256
                or view.observation_manifest_sha256 != request.observation_manifest_sha256
                or view.observation_evidence_sha256 != request.observation_evidence_sha256
                or view.queue_watermark != receipt_value["queue_watermark"]
                or view.completed_watermark != view.queue_watermark
                or view.semantic_completeness != "not_required"
            ):
                raise GenerationConflict("promoted queue binding differs from structural authority")
        receipt_value = receipt.to_dict()
        ...
        if (
            ...
            or receipt_value["semantic_completeness"] != "not_required"
            or state.completion_binding is None
            or receipt_value["completion_binding"] != state.completion_binding.to_dict()
            ...
            or {entry["path"] for entry in payload_entries}
            ...
            for name in names:
                ...
                try:
                    ...
                    try:
                        ...
                        raise StatePathError(
                            "semantic release decision state is unsupported in S3"
                        ) from exc
    ...
    def _payload_completion_binding(
        self,
        container: Path,
        entries: Sequence[Mapping[str, object]],
        initial: InputManifest,
        observed_consumed: InputManifest,
    ) -> CompletionBinding:
        files = {str(entry["path"]): entry for entry in entries}
        if set(files) != {"graphify-out/graph.json", "graphify-out/input-manifest.json"}:
            raise GenerationConflict("structural payload requires exactly graph and input manifest")
        payload = self.state.read_contained_regular_file(
            container.relative_to(self.state.root), "graphify-out/input-manifest.json",
            max_bytes=MAX_DOCUMENT_BYTES,
            allowed_directory_modes=_ALLOWED_DIRECTORY_MODES,
            allowed_file_modes=_ALLOWED_FILE_MODES,
        )
        if hashlib.sha256(payload).hexdigest() != files["graphify-out/input-manifest.json"]["sha256"]:
            raise PayloadChanged("input manifest changed during payload inspection")
        try:
            consumed = InputManifest.from_json(payload)
            if consumed.canonical != observed_consumed.canonical:
                raise GenerationConflict(
                    "staged consumed inputs differ from trusted source observations"
                )
            return CompletionBinding.bind(
                initial, consumed, compatibility=self.compatibility_manifest,
                graph_sha256=str(files["graphify-out/graph.json"]["sha256"]),
            )
        except ContractError as exc:
            raise GenerationConflict(f"structural input completion is invalid: {exc}") from exc

    def _require_sealed_completion(
        self, state: StagedBuildState, container: Path, entries: Sequence[Mapping[str, object]]
    ) -> None:
        binding = state.completion_binding
        if binding is None:
            raise GenerationConflict("completed stage has no input binding")
        files = {str(entry["path"]): entry for entry in entries}
        if set(files) != {"graphify-out/graph.json", "graphify-out/input-manifest.json"}:
            raise GenerationConflict("sealed structural payload contains unexpected files")
        payload = self.state.read_contained_regular_file(
            container.relative_to(self.state.root), "graphify-out/input-manifest.json",
            max_bytes=MAX_DOCUMENT_BYTES,
            allowed_directory_modes=_ALLOWED_DIRECTORY_MODES,
        ...
        if (consumed.sha256 != values["consumed_inputs_sha256"]
                or values["graph_sha256"] != files["graphify-out/graph.json"]["sha256"]
                or values["initial_detection_sha256"] != state.request.observation_manifest_sha256
                or values["compatibility_sha256"] != self.compatibility_sha256):
            raise PayloadChanged("sealed input completion differs from staged authority")
    ...
    def _reuse_staged_completion_locked(
        ...
        *,
        ...
        self._require_sealed_completion(state, container, inventory.entries)
    ...
    def complete_staged_build(
        ...
        *,
        ...
        trusted_observations = self._require_structural_evidence(
            repo_uuid,
            request,
            source_observations,
        )
        observed_consumed = trusted_observations[0].consumed_inputs
        if observed_consumed is None:
            raise GenerationConflict("trusted source observations lack consumed inputs")
        with self.leases.current_operation(
            preparation.grant,
            monotonic_ns=monotonic_ns,
            allowed_operations=frozenset({"BUILD"}),
            ...
            with self.state.existing_generation_lock(
                ...
                if payload_bytes <= preparation.allocation.expected_payload_bytes:
                    ...
                    initial = source_observations[0].initial_detection
                    if initial.sha256 != request.observation_manifest_sha256:
                        raise GenerationConflict("completion initial evidence differs from request")
                    binding = self._payload_completion_binding(
                        preparation.allocation.staging_path, reinventory.entries, initial,
                        observed_consumed,
                    )
    ...
    def _receipt(
        ...
        staged = self._load_staged_build_locked(operation.repo_uuid)
        if (staged is None or staged.generation_id != allocation.generation_id
                or staged.completion_binding is None):
            raise GenerationConflict("certification lacks durable S2 completion binding")
        return cast(
            GenerationReceipt,
            GenerationReceipt.from_mapping(
                {
                    ...
                    "completion_binding": staged.completion_binding.to_dict(),
    ...
    def verify_generation(
        ...
        *,
        ...
        sealed_inputs = self.state.read_contained_regular_file(
            relative, "graphify-out/input-manifest.json",
            max_bytes=MAX_DOCUMENT_BYTES,
            allowed_directory_modes=_ALLOWED_DIRECTORY_MODES,
            allowed_file_modes=_ALLOWED_FILE_MODES,
            deadline_ns=deadline_ns,
        ...
        try:
            consumed = InputManifest.from_json(sealed_inputs)
        except ContractError as exc:
            raise GenerationError(f"sealed input manifest is invalid: {exc}") from exc
        completion = cast(Mapping[str, object], value["completion_binding"])
        if (not consumed.complete
                or consumed.sha256 != completion["consumed_inputs_sha256"]):
            raise GenerationError("sealed input manifest differs from certification")
        _require_verification_deadline(deadline_ns)
        queue_watermark = int(value["queue_watermark"])
        validations = tuple(
        ...
        *,
        ...
        if queue_view is None:
            ...
            if staged_completion is not None:
                completion_binding = staged_completion.state.completion_binding
                consumed = trusted_observations[0].consumed_inputs
                if (
                    completion_binding is None
                    or consumed is None
                    or consumed.sha256
                    != completion_binding.to_dict()["consumed_inputs_sha256"]
                ):
                    raise SemanticCertificationBlocked(
                        "trusted consumed inputs differ from staged completion"
                    )
