...
class SemanticQueuePolicy:
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "max_claimed_tasks": self.max_claimed_tasks,
        }
    ...
    def from_mapping(cls, value: Mapping[str, object]) -> "SemanticQueuePolicy":
        ...
        if _integer(data["format_version"], "$.format_version", minimum=1) != 2:
        ...
        return cls(
            ...
            max_claimed_tasks=_integer(data["max_claimed_tasks"], "$.max_claimed_tasks", minimum=1),
        )
...
class SemanticQueueStore:
    ...
    def _bounded(self, snapshot: SemanticQueueSnapshot) -> None:
        ...
        if sum(item.status == "claimed" for item in snapshot.items) > self.policy.max_claimed_tasks:
            raise SemanticQueueCapacityExceeded("claimed task capacity would be exceeded")
