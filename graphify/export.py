...
def backup_if_protected(out_dir: Path) -> "Path | None":
    ...
    if os.environ.get("GRAPHIFY_NO_BACKUP"):
        return None
    out = Path(os.path.realpath(Path(out_dir).expanduser().absolute()))
    ...
    require_ordinary_output(backup_dir)
    ...
    try:
        backup_dir = ordinary_mkdir(backup_dir)
        copied = 0
        for name in _BACKUP_ARTIFACTS:
            src = out / name
            if src.exists():
                try:
                    with src.open("rb") as source, ordinary_open(backup_dir / name, "wb") as target:
...
def to_json(G: nx.Graph, communities: dict[int, list[str]], output_path: str, *, force: bool = False, built_at_commit: str | None = None, community_labels: dict[int, str] | None = None) -> bool:
    ...
    try:
        serialized = ordinary_temporary_file(Path(output_path).parent)
    ...
    with serialized:
        ...
        serialized.seek(0)
        require_ordinary_output(output_path)
        with ordinary_open(output_path, "w", encoding="utf-8") as f:
...
def to_obsidian(
    ...
    """
    out = Path(output_dir)
    require_ordinary_output(out)
    out = ordinary_mkdir(out)
    ...
    def _owned_write(rel_name: str, content: str) -> bool:
        ...
        with ordinary_open(target, "w", encoding="utf-8") as stream:
    ...
    try:
        require_ordinary_output(_manifest_path)
        with ordinary_open(_manifest_path, "w", encoding="utf-8") as stream:
            stream.write(json.dumps({"files": sorted(set(_written))}, indent=2))
    except OSError:
        pass
    if _skipped:
...
def to_canvas(
    ...
    with ordinary_open(output_path, "w", encoding="utf-8") as stream:
        stream.write(json.dumps(canvas_data, indent=2))
...
def to_graphml(
    ...
    try:
        require_ordinary_output(tmp)
        with ordinary_open(tmp, "wb") as stream:
            nx.write_graphml(H, stream)
        require_ordinary_output(out)
        ordinary_replace(tmp, out)
    finally:
        try:
            ordinary_unlink(tmp)
...
def to_svg(
    ...
    plt.tight_layout()
    require_ordinary_output(output_path)
    with ordinary_open(output_path, "wb") as stream:
        plt.savefig(stream, format="svg", bbox_inches="tight",
