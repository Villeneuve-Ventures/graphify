...
provisioning prerequisite is now contract-frozen only and remains `WAITING`,
not `READY` or `COMPLETE`. Its future private store owns the exact
...
vocabulary and revocation or reactivation is not authorized. No live release
context, profiles, coverage result, policy mapping, or operator record is
selected or provisioned by this documentation freeze.
...
bytes, implementation and acceptance of the separately frozen but still
`WAITING` policy-authority provisioning prerequisite, a provisioned stable
...
| Semantic-release policy-authority provisioning | Unnumbered P5B2 prerequisite (`WAITING`; contract freeze only) | The [frozen internal provisioning boundary](semantic-sync.md#p5b2-semantic-release-policy-authority-provisioning-prerequisite) gives future `SemanticReleasePolicyAuthorityStore` sole ownership of the private current/previous/pending records, exact `ACTIVE` selection input and digest preimages, revision-plus-one/predecessor CAS, shared-read/exclusive-write registry-then-workspace locking, fixed 256 KiB transaction peak, durable commit/recovery, commit-uncertainty, and byte-identical replay rules. It provisions no live record. `SELECT_SEMANTIC_RELEASE_POLICY` cannot revoke or reactivate; `REVOKED` remains consumer-side fail-closed vocabulary. Implementation, readiness, acceptance, public surfaces, decision binding, composition, omission, projection, provider/backend, publication, and successor activation remain absent. |
...
- [Host-agent worker, semantic-generation, trust-root, policy-authority provisioning, and release-decision contracts](semantic-sync.md)
