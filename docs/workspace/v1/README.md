...
P5B2 semantic-release policy-authority provisioning prerequisite, the accepted
unnumbered P5B2 semantic-release decision-store and capacity/GC prerequisite,
...
The internal P5B2 semantic-result handoff and sealed-input finalization child is
implemented and accepted only at the boundary below. Its completion evidence is
the [P5B2 semantic-result handoff acceptance receipt](receipts/p5b2-semantic-result-handoff.md).
...
only that request's existing staged-recovery authority: `PROMOTE` for an exact
new pointer move or already-visible replay, and `POINTER_RECOVERY` only when
exact durable pending intent remains from that same move. It stops only after
staged `PROMOTED`, the exact visible current pointer and authoritative
promotion/recovery journal, no pending pointer intent, unchanged installed
semantic generation evidence, and release of the exact promotion owner/fence
...
The separate unnumbered P5B2 semantic-release bundle and
deterministic-classifier trust-root prerequisite is implemented and accepted
only at the frozen internal boundary below. Its completion evidence is the
[P5B2 semantic-release trust-root acceptance receipt](receipts/p5b2-semantic-release-trust-root.md),
which binds the exact PR #66 contract freeze, PR #67 readiness reconciliation,
PR #68 implementation, and PR #69 C1-control repair. Its exact internal scope
is the repo-owned installed
...
override those bytes. Acceptance adds no content-release, publication,
...
transaction peak, and the existing pending→previous→current→pending-clear
...
which binds PR #76's contract freeze, PR #77's implementation delivery, PR
#79's held generation-directory correction, and PR #83's top-level
decision-namespace correction. The boundary gives
...
byte-identical replay behavior, commit-uncertainty handling, and the rule that
nonempty decision state blocks shared workspace reachability for GC. Acceptance
provisions no live decision and grants no canonical-state cleanup, repair,
deletion, quarantine, rollback, public surface, runtime receipt, publication,
parent completion, or successor authority.

First visibility is built in one fixed mode-`0700`, non-authoritative
`semantic-release-decision-publication` slot outside the canonical decision
namespace and outside lifecycle `staging`. The slot holds at most one bounded
build/ready/cleanup state; an exclusive no-overwrite rename publishes the first
missing root, generation, or binding boundary. Retry may remove only validated
noncanonical residue after proving the encoded destination absent or
byte-identical. The 4 KiB manifest and 256 KiB transient staging allowance are
fixed internal bounds; logical capacity continues to charge only canonical
binding bytes.
...
remain outside the accepted trust-root boundary. P5 and P5B2 remain
...
The accepted unnumbered P5B2 child is internal-only. It adds no public argv,
...
The unnumbered P5B2 semantic-generation certification finalization child is
implemented and accepted only within this frozen internal contract. Its only
entry is the exact accepted handoff terminal state: the
canonical `SyncRequest` and target generation still bind one request-bound staged
...
| Semantic-release decision-store and capacity/GC | Unnumbered P5B2 prerequisite (`COMPLETE`) | The [accepted private persistence boundary](semantic-sync.md#p5b2-semantic-release-decision-store-and-capacitygc-prerequisite) owns the request-addressed mode-`0600` decision binding, one bounded non-authoritative publication-staging slot, exclusive first-boundary visibility, exact member sets and digest preimages, bounded per-binding/generation/workspace accounting, install-once/replay and commit-unknown rules, existing lock-order integration, authoritative capacity charging, workspace-wide pre-plan GC blocking, and internal runtime composition with direct hostile and failure-injection tests. Completion evidence: [`P5B2 semantic-release decision store and capacity/GC`](receipts/p5b2-semantic-release-decision-store-capacity-gc.md), binding PR #76, PR #77, PR #79, and PR #83. Acceptance provisions no live decision and grants no classification composition, canonical-state cleanup, repair, deletion, quarantine, rollback, public surface, publication, parent completion, or successor authority. |
| Semantic-content release/DLP decision | Proposed unnumbered P5B2 child (`WAITING`; contract freeze only) | The [frozen private decision boundary](semantic-sync.md#p5b2-semantic-content-releasedlp-decision) requires the accepted exact promoted visible-current terminal, accepted trust-root, policy-authority provisioning, and decision-store/capacity/GC mechanisms, a provisioned stable current `ACTIVE` operator policy authority, and classification composition. Omission execution, projection, public CLI/schema/runtime receipt, provider/backend, publication, implementation, acceptance, and successor activation remain absent. |
...
- `graphify.workspace.persistence`, `.identity`, `.registry`, `.leases`,
  ...
  staging, validation, and queue-transition composition. The accepted handoff
...
- [Host-agent worker, semantic-generation, trust-root, policy-authority provisioning, decision-store, and release-decision contracts](semantic-sync.md)
